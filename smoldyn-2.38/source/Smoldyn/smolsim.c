...
int simreadstring(simptr sim,ParseFilePtr pfp,const char *word,char *line2) {
	...
	int er,dim,i,nmol,d,i1,s,c,ll,order,more,*index;
	int rctident[MAXORDER],nprod,prdident[MAXPRODUCT],rulelist[MAXPRODUCT],r,ord,prd,itct,prt,lt,f;
	...
	if(!strcmp(word,"dim")) {											// dim
		...
		itct=sscanf(line2,"%s %lg %lg",nm,&flt1,&flt2);
		...
		itct=sscanf(line2,"%s %lg %c",nm,&flt1,&ch);
